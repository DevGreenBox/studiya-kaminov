...
export function EmptyState({ icon, title, text, action, secondaryAction }: Props) {
  return (
    <div className="flex flex-col items-center justify-center rounded-[var(--radius-lg)] border border-line bg-surface px-6 py-14 text-center">
      {icon && (
        <div
          className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-white text-ink-muted"
          aria-hidden
        >
          {icon}
        </div>
      )}
      <h2 className="text-xl font-bold">{typo(title)}</h2>
      {text && (
        <p className="mt-2 max-w-md text-[15px] leading-relaxed text-ink-soft">{typo(text)}</p>
      )}
